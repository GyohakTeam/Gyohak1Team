...
export default function ClassroomPanel({
  classrooms, workers, selectedWorkerId, hoveredWorkerId,
  mode, status, events, onImport, onClear, onSwitchToPaste, onInspectorClick, onAutoAssign, onUpdateClassroom,
  ...
  const hoverWorker = workers.find(w => w.id === hoveredWorkerId);
  const selColor    = selWorker ? getPersonColor(selWorker.name) : null;

  const rowClass = (classroom) => {
    if (!hoverWorker) return 'classroom-row';
    return canInspect(hoverWorker, classroom)
      ? 'classroom-row hover-available'
      : 'classroom-row hover-unavailable';
  };

  const rowStyle = (classroom) => {
    if (selWorker && classroom.inspectorId === selectedWorkerId) {
      return {
        boxShadow: `inset 5px 0 0 ${selColor}`,
        ...
        <div className="mode-wrap">
          ...
          <div className="paste-content">
            <div className="paste-hint">
              HWP·메모장 등에서 복사 후 아래에 <strong>붙여넣기(Ctrl+V)</strong> 하면 자동으로 표가 채워집니다.<br />
              세로형: 강의실명 한 줄 → 시간대 한 줄씩 &nbsp;|&nbsp; 가로형: 탭 또는 <code>|</code> 구분
            </div>
            <textarea
              ref={textareaRef}
            ...
            />
            {status.text && (
              <div className="status-bar" style={{ borderLeftColor: STATUS_BORDER[status.type] }}>
                {status.text}
              </div>
            )}
            <div className="btn-row">
              ...
              <button className="btn btn-secondary" onClick={handleClear}>초기화</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="mode-wrap">
          <div className="panel-title">
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              {workers.length > 0 && (
                <button className="reload-btn" onClick={onAutoAssign}>⚡ 자동 배정</button>
              )}
              <button className="reload-btn" onClick={() => setShowEventForm(v => !v)}>
                {showEventForm ? '✕ 닫기' : '+ 행사추가'}
              </button>
              <button className="reload-btn" onClick={onSwitchToPaste}>↩ 다시 불러오기</button>
            </div>
          </div>
            ...
            <div className="event-panel">
              <div className="event-form-row">
                ...
                <input
                  ...
                  onKeyDown={e => e.key === 'Enter' && handleAddEvent()}
                />
                <button className="btn btn-primary btn-sm" onClick={handleAddEvent}>추가</button>
              </div>
              {events.length > 0 && (
              ...
              <tbody>
                {classrooms.length === 0 ? (
                  ...
                  const title = selectedWorkerId
                    ? (assignable ? '클릭하여 배정' : '근무시간 불일치')
                    : (inspector ? `${inspector.name} — 클릭 시 선택` : '근로자를 먼저 선택하세요');

                  return (
                    <tr key={c.id} className={rowClass(c)} style={rowStyle(c)}>
                        ...
                        {isEditing ? (
                          <div className="time-edit">
                            ...
                            <div className="time-edit-btns">
                              <button className="btn btn-primary btn-sm" onClick={() => saveEdit(c.id)}>저장</button>
                          ...
                          <div className="time-chips">
                            {c.timeSlots.length > 0
                              ? c.timeSlots.map((t, i) => <span key={i} className="time-chip">{t}</span>)
                              : <span className="no-time">없음</span>}
                            <button className="edit-slot-btn" onClick={() => startEdit(c)} title="시간 수정">✏</button>
                            {roomEvents.map(ev => (
                      ...
                      <td
                        className={inspClass}
                        onClick={() => !isEditing && onInspectorClick(c.id)}
                        title={title}
                        style={inspector ? { backgroundColor: getPersonColor(inspector.name) } : {}}
                      >
                        {inspector
                          ? <div className="inspector-name" style={{ color: '#111' }}>{inspector.name}</div>
